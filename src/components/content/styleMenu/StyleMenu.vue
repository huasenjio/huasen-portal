...
<script>
import { getStyleById, setStyleById, removeStyleById } from '@/plugin/styleStore.js';
export default {
  name: 'StyleMenu',
  props: {
    bindingData: {
  ...
  methods: {
    changeStyle(val, tag) {
      this.$data[tag] = val;
      let style = getStyleById(this.bindingData.id);
      style[tag] = val;
      setStyleById(this.bindingData.id, style);
    ...
    reset() {
      removeStyleById(this.bindingData.id);
      this.$store.dispatch('snapshoot');
